...
pub fn run() {
    ...
    #[cfg(debug_assertions)]
    builder
        .export(specta_typescript::Typescript::default(), "../src/bindings.d.ts")
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let identity = crypto::init()
                .expect("Failed to initialize cryptographic identity");
            ...
            let tcp_port = {
                ...
                db::settings::get_peer_settings(&db_guard)
                    .map(|s| s.listen_port as u16)
                    .unwrap_or(9473)
            };
            network::tcp_server::start_tcp_server(
                tcp_port,
                app.handle().clone(),
                conn_mgr_clone,
            );
