...
#[tauri::command]
#[specta::specta]
pub fn start_discovery(state: tauri::State<'_, crate::AppState>, app: tauri::AppHandle) -> Result<String, String> {
    ...
    *state.heartbeat_sender.lock().map_err(|e| e.to_string())? = Some(heartbeat_state.sender());
    ...
    crate::network::heartbeat::start_heartbeat_monitor(
        app.clone(),
        heartbeat_state,
        heartbeat_rx,
    );
...
pub async fn connect_manual(
    ...
    }).await.map_err(|_| "Handshake timed out".to_string())?
    ...
    let peer_row = peers.into_iter()
        .find(|p| p.fingerprint == their_fingerprint)
        .ok_or("Failed to find newly connected peer".to_string())?;
    
    // Emit discovery event
    let _ = app.emit("peer:discovered", crate::network::discovery::PeerDiscoveredPayload {
    ...
    // Notify heartbeat
    if let Some(sender) = state.heartbeat_sender.lock().map_err(|e| e.to_string())?.as_ref() {
        let _ = sender.send(crate::network::heartbeat::HeartbeatAction::Seen(peer_row.fingerprint.clone()));
    }
