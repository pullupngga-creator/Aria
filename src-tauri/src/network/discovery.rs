...
pub fn start_mdns_broadcast(
    ...
    println!("mDNS broadcast started for {} on port {}", instance_name, listen_port);
    ...
    tauri::async_runtime::spawn(async move {
        let receiver = match daemon.browse("_aria._tcp.local.") {
            ...
            Err(e) => {
                ...
                let _ = app_handle.emit("network:mdns_disabled", serde_json::json!({
            ...
            match event {
                ServiceEvent::ServiceResolved(info) => {
                    ...
                    if let Some(fp) = fingerprint {
                        ...
                        let display_name = properties.get_property_val_str("name").map(|s| s.to_string());
                        let public_key = properties.get_property_val_str("pubkey").unwrap_or(fp).to_string();
                        
                        // Get all valid IP addresses
                        let all_addresses: Vec<std::net::IpAddr> = info.get_addresses()
                            .into_iter()
                            .cloned()
                            .collect();
...

                        {
                            ...
                            let network_interface = primary_ip.as_deref()
                                .and_then(|ip| ip.split('.').take(3).collect::<Vec<_>>().join(".").into());
                        ...
                        let _ = app_handle.emit("peer:discovered", PeerDiscoveredPayload {
                ...
                ServiceEvent::ServiceRemoved(_, fullname) => {
                    ...
                    if let Some(instance_name) = parts.first() {
                        let fp = instance_name.split('-').last();
                        if let Some(fingerprint) = fp {
                            ...
                            let _ = app_handle.emit("peer:offline", PeerDiscoveredPayload {
